            ...
            <div
                className={`flex flex-col items-center transition-all duration-700 ease-out ${
                    hasSearched && results.length > 0
                        ? "py-8 min-h-[0vh]"
                        : "py-32 justify-center min-h-[60vh]"
                }`}
            >
                <h1
                    className={`text-4xl md:text-6xl font-black tracking-tighter text-foreground mb-2 text-center transition-all ${
                        hasSearched && results.length > 0 ? "hidden" : "block"
                    }`}
                >
                    Find Your{" "}
                    <span className="relative inline-block pr-6">
                        <span
                            className="absolute inset-0 text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-purple-600 blur-lg opacity-80 select-none pr-2 pb-1"
                            aria-hidden="true"
                        >
                            Scene
                        </span>
                        <span className="relative text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-purple-600 font-black pr-2 pb-1">
                            Scene
                        </span>
                    </span>
                </h1>
                ...
                <p
                    className={`text-muted-foreground text-lg mb-8 text-center max-w-lg ${
                        hasSearched && results.length > 0 ? "hidden" : "block"
                    }`}
                >
                    Discover the real-world locations behind your favorite
                    cinematic moments.
                </p>
                ...
                {!loading && !hasSearched && !results.length && (
                    <div
                        className="mt-8 flex gap-4 animate-fade-in"
                ...
                {!loading && hasSearched && results.length === 0 && (
                    <div className="mt-12 text-center animate-fade-in">
                        ...
                        <p className="text-muted-foreground max-w-sm mx-auto">
                            ...
                            {searchedQuery}". Try different keywords or browse
                            our collection.
                        </p>
                        <div className="mt-6">
                            <Link to="/browse?shuffle=1">
