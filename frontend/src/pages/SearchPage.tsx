    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className={`flex flex-col items-center transition-all duration-700 ease-out ${results.length > 0 ? 'py-8 min-h-[0vh]' : 'py-32 justify-center min-h-[60vh]'}`}>
                ...
                <h1 className={`text-4xl md:text-6xl font-black tracking-tighter text-white mb-2 text-center transition-all ${results.length > 0 ? 'hidden' : 'block'}`}>
                    Find Your <span className="text-cyan-400 text-glow">Scene</span>
                </h1>

                <p className={`text-zinc-400 text-lg mb-8 text-center max-w-lg ${results.length > 0 ? 'hidden' : 'block'}`}>
                ...
                <div className="w-full max-w-2xl mx-auto relative group z-20">
                    ...
                    <div className="relative z-10 flex items-center bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-xl p-2 shadow-2xl">
                        <Search className="w-6 h-6 text-zinc-500 ml-3" />
                        <input
                            type="text"
                            className="w-full bg-transparent border-none outline-none focus:outline-none focus:ring-0 text-white placeholder-zinc-500 text-lg px-4 py-2"
                        ...
                        />
                        <div className="flex items-center gap-2 pr-2 border-l border-white/5 pl-2">
                            <select
                                className="bg-transparent text-zinc-400 text-sm focus:outline-none cursor-pointer hover:text-white"
                                value={k}
                            ...
                            <button
                                onClick={doSearch}
                                disabled={loading}
                                className="px-6 py-2.5 bg-cyan-500 hover:bg-cyan-400 text-black font-bold rounded-lg transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.5)] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {loading ? (
                ...
                {!results.length && !loading && (
                    <div className="mt-8 flex gap-4 animate-fade-in" style={{ animationDelay: '0.2s' }}>
                        <Link to="/browse?shuffle=1">
                            <button className="flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 text-sm text-zinc-300 transition-colors">
